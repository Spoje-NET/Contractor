...
if (empty($kod)) {
    ...
    try {
        ...
        $oPage->body->addItem(new Ui\ContractForm($contractor, true));

        switch ($format) {
            case 'pdf':
                header('Content-Type: application/pdf');
                $filename = _('Smlouva').'-'.\AbraFlexi\Functions::uncode($contractor->getRecordCode()).'.pdf';
            ...
            case 'docx':
                ...
                $filename = _('Smlouva').'-'.\AbraFlexi\Functions::uncode($contractor->getRecordCode()).'.docx';
